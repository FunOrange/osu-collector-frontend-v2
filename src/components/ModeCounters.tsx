      ...
      {(['osu', 'taiko', 'mania', 'fruits'] as const).map((mode) => {
        ...
        return (
          <div key={mode} className='flex items-center'>
            ...
            />
            <div className={nonZero ? undefined : 'text-gray-500'}>{collection.modes?.[mode] ?? 0}</div>
          </div>
        );
      })}
      {showUnavailable && (collection.unsubmittedBeatmapCount > 0 || collection.unknownChecksums?.length > 0) && (
        <TooltipProvider delayDuration={0}>
          <Tooltip>
            ...
            <TooltipContent>
              {collection.unsubmittedBeatmapCount > 0 && <div>{collection.unsubmittedBeatmapCount} unsubmitted</div>}
              {collection.unknownChecksums?.length > 0 && <div>{collection.unknownChecksums.length} processing</div>}
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
      )}
