...
export default function SearchInput({ searchParams, withIcon }: SearchInputProps) {
  ...
  if (withIcon) {
    return (
      <div className='relative'>
        ...
        <input
          className='px-3 py-3 text-xl border rounded-md pl-11 border-slate-900 bg-[#162032] ring-offset-background placeholder:text-muted-foreground'
  ...
  return (
    <input
      className='px-3 py-2 text-2xl text-center border rounded-md border-slate-900 bg-[#162032] shadow-inner ring-offset-background placeholder:text-muted-foreground'
      placeholder='Search...'
      value={userInput}
      onChange={(e) => setUserInput(e.target.value)}
      onKeyDown={(e) => {
