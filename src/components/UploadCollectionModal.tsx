...
export default function UploadCollectionModal({ children, open, onOpenChange }: UploadCollectionModalProps) {
      ...
      <DialogContent className='max-w-4xl'>
        ...
        <div className='mt-4 grid' style={{ gridTemplateRows: 'auto auto 1fr auto', height: '85vh' }}>
          <div className={cn('flex flex-col gap-2', localCollections === undefined ? undefined : 'opacity-60')}>
            ...
            >
              <input {...getInputProps()} />
              <span>{isDragActive ? 'Drop the file here...' : filename || 'Open collection.db...'}</span>
            </div>
          </div>
              ...
              <div className='flex flex-col divide-y overflow-auto'>
                ...
                {localCollections.map((collection, i) => (
                  <div
                    ...
                    onClick={() => setSelectedCollection((prev) => (prev === collection ? undefined : collection))}
                  >
                    <div className='flex items-center justify-between'>
                      <div>
                        {collection.name}
                        <span className='ml-3 text-slate-500'>{collection.beatmapChecksums.length} beatmaps</span>
                      </div>
