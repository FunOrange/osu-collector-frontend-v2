...
export const checkRuleAndSetError = (
  rule: ValidationRule,
  fields: any,
  fieldName: string,
  setErrors: any
...
const rules = {
  ...
  requiredAndNotNullWhen: (condition: boolean) =>
    rule(
      (input) => condition && (input === "" || input === undefined || input === null),
      "This field is required"
    ),
  ...
  integer: () =>
    rule((input) => input !== "" && !Number.isInteger(Number(input)), "Must be a whole number"),
  nonNegative: () =>
    rule((input) => input !== "" && Number(input) < 0, "Must be a positive number"),
  positive: () => rule((input) => input !== "" && Number(input) <= 0, "Must be a positive number"),
  max: (max: number) =>
    rule(
      (input) => input !== "" && Number(input) >= max,
      `Must be less than ${(max + 1).toLocaleString()}`
    ),
  min: (min: number) =>
    rule(
      (input) => input !== "" && Number(input) <= min,
      `Must be greater than ${(min - 1).toLocaleString()}`
    ),
  minLength: (min: number) =>
    rule(
      (input) => input.trim().length > 0 && input.trim().length < min,
      `Must be at least ${min} characters`
    ),
  maxLength: (max: number) =>
    rule((input) => input.length > max, `Must be less than ${max} characters`),
  email: () =>
    rule(
      (input) => !/^[\w\-\.]+(\+[\d\w]+)?@[a-z]+\.[a-z]{2,3}$/.test(input),
      "Not a valid email address"
    ),
  emailWithoutDomain: () => rule((input) => input.includes("@"), "Please use the dropdown for @"),
  emailUsername: () =>
    rule((input) => !/^[\w-\.]+(\+\d+)?$/.test(input), "Not a valid email address"),
  phone: () =>
    rule(
      (input) =>
  ...
  suppliedByGoogleAutocomplete: () =>
    rule((input) => !input.trim(), "Please use Address Line 1 to select an address"),
  hasCapitalLetter: () => rule((input) => !/[A-Z]/.test(input), "Must contain a capital letter"),
  hasLowercaseLetter: () =>
    rule((input) => !/[a-z]/.test(input), "Must contain a lowercase letter"),
