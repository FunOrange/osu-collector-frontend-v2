...
export const calculateOverlapWithTarget = (target: [number, number], value: [number, number]) => {
  ...
  const valueLength = vEnd - vStart;
  return valueLength > 0 ? overlapLength / valueLength : 0;
};
